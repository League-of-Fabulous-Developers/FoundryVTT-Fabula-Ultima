...
function attachSupportCheckListener(message, jQuery)
{
    ...
    if (groupCheck)
    {
        jQuery.find(`button[data-support=${groupCheck.id}]`).click(async () =>
        {
            const character = canvas.tokens.controlled.at(0)?.document.actor || game.user.character;
            if (character && groupCheck.leader !== character.id && !groupCheck.supporters.find((supporter) => supporter.id === character.id))
            {
            ...
            {
                ui.notifications.error('You already supported this Group Check!');
            }
        });
    }
}
...
export class GroupCheck extends Application
{
    ...
    constructor(groupCheckId)
    {
        ...
        this.#chatMessage = game.messages.search({
            filters: [
                {
    ...
    async close(options = {roll: false})
    {
