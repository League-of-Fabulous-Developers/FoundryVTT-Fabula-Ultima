...
export class CustomWeaponDataModel extends PseudoDocumentEnabledTypeDataModel {
	...
	async renderInlay() {
		...
		element.innerHTML = `
          ...
          ${
				this.isTransforming
					? `<div class="flex0" data-action="changeCustomWeaponForm" data-tooltip="${game.i18n.localize('FU.CustomWeaponChangeForm')}">
				${this.activeForm === 'primaryForm' ? `<span>Ⅰ</span>` : `<span>Ⅱ</span>`}
		 	</div>`
					: ''
			}
          <div class="flex0">
            <a data-action="equipCustomWeapon" data-item-id="${this.parent.id}">
              ${this.parent.isEquipped ? '<i class="ra ra-crossed-swords"></i>' : '<i class="far fa-circle"></i>'}
            </a>
          </div>
        `;
		element.querySelector('[data-action=equipCustomWeapon]').addEventListener('click', (e) => {
			this.parent.actor.update({
				'system.equipped.mainHand': this.parent.isEquipped ? null : this.parent.id,
				'system.equipped.offHand': this.parent.isEquipped ? null : this.parent.id,
			});
			return false;
		});
		element.querySelector('[data-action=changeCustomWeaponForm]').addEventListener('click', (e) => {
			this.parent.update({
