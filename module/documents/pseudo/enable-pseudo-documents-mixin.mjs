...
/**
 * @param {foundry.abstract.TypeDataModel} TypeDataModel
 * @return {typeof TypeDataModelPseudoDocumentsMixin}
 */
export function forTypeDataModel(TypeDataModel) {
	if (!foundry.utils.isSubclass(TypeDataModel, foundry.abstract.TypeDataModel)) {
		throw new Error(`${TypeDataModel.name} is not a TypeDataModel.`);
	}

	class TypeDataModelPseudoDocumentsMixin extends TypeDataModel {
		_configure() {
	...
	return TypeDataModelPseudoDocumentsMixin;
}
...
export const EnablePseudoDocuments = {
	forTypeDataModel: forTypeDataModel,
