...
export class FUActorSheet extends ActorSheet {
  ...
  static get defaultOptions () {
    return mergeObject(super.defaultOptions, {
      ...
      template:
        'systems/fabulaultima/templates/actor/actor-character-sheet.html',
      width: 600,
      height: 1150,
      tabs: [
        {
  ...
  _prepareItems (context) {
    ...
    for (let i of context.items) {
      ...
      i.equippedSlot =
        i.system.isEquipped && i.system.isEquipped.slot ? true : false
  ...
  activateListeners (html) {
    ...
    function handleItemClick (ev, isRightClick) {
      ...
      icon
        .removeClass('fa-toolbox')
        .addClass(getIconClassForEquippedItem(item, itemType))
      ...
      console.log(
        `${slot} Equipped Items:`,
        this.actor.items
          .filter(i => i.system.isEquipped && i.system.isEquipped.slot === slot)
          .map(i => i.name)
      )
    }
    ...
    function getIconClassForEquippedItem (item, itemType) {
      if (
        ...
        itemType === 'weapon' &&
        item.system.hands.value === 'two-handed'
      ) {
        return 'ra ra-sword ra-2x'
      } else if (
        item.system.isEquipped.slot === 'mainHand' &&
        itemType === 'weapon' &&
        item.system.hands.value === 'one-handed'
      ) {
        return 'ra ra-plain-dagger ra-2x'
      } else if (
        item.system.isEquipped.slot === 'offHand' &&
        itemType === 'weapon'
      ) {
        return 'ra ra-crossed-swords ra-2x'
      } else if (item.system.isEquipped.slot === 'mainHand') {
        return item.type === 'shield' &&
          item.system.isDualShield &&
          item.system.isDualShield.value
          ? 'ra ra-heavy-shield'
          : 'ra ra-shield'
    ...
    html.find('.increment-button, .decrement-button').click(ev => {
      const currentSheet = $(ev.currentTarget).closest(
        '.fabulaultima-actor-sheet'
      )
    ...
    function generateStarsHTML (value, max) {
      ...
      for (let i = 0; i < max; i++) {
        starsHTML += `<div class="star ${
          i < value ? 'fus-sl-star' : 'ful-sl-star'
        }"></div>`
      }
    ...
    const behaviors = this.actor.items.filter(
      item =>
        [
          ...
          'miscAbility',
          'behavior'
        ].includes(item.type) && item.system.isBehavior?.value
    )
  ...
  async _openCheck (rollType) {
    ...
    const secondaryAttributeValue = getProperty(
      actorData,
      secondaryAttributeRef
    )
    ...
    const primaryAttributeName =
      game.i18n.localize(
        CONFIG.FU.attributeAbbreviations[primaryAttributeKey]
      ) ?? primaryAttributeKey
    const secondaryAttributeName =
      game.i18n.localize(
        CONFIG.FU.attributeAbbreviations[secondaryAttributeKey]
      ) ?? secondaryAttributeKey

    // Perform the roll calculation here using the extracted attribute values
    const primaryRollResult = await new Roll(
      `1d${primaryAttributeValue}`
    ).evaluate({ async: true })
    const secondaryRollResult = await new Roll(
      `1d${secondaryAttributeValue}`
    ).evaluate({ async: true })
    ...
    const totalResult =
      primaryRollResult.total + secondaryRollResult.total + bonuses
  ...
  _onRoll (event) {
    ...
    if (dataset.rollType) {
      ...
      if (
        dataset.rollType === 'roll-check' ||
        dataset.rollType === 'roll-init'
      ) {
    ...
    if (dataset.rollType === 'item-slot') {
      ...
      const sameSlotItems = items.filter(
        i => i.system.isEquipped && i.system.isEquipped.slot === slot
      )
