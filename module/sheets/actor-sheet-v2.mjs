...
/**
 * @description Clean-slate v2 actor sheet for Project FU
 * @property {FUActor} actor
 * @extends {FUActorSheet}
 */
export class FUActorSheetV2 extends FUActorSheet {
	/**
	 * @inheritDoc
	...
	static DEFAULT_OPTIONS = {
		...FUActorSheet.DEFAULT_OPTIONS,
		classes: [...FUActorSheet.DEFAULT_OPTIONS.classes, 'projectfu-v2'],
		position: {
			width: 800,
			height: 1000,
		},
		actions: {
			roll: FUActorSheetV2.Roll,
			spendMetaCurrency: FUActorSheetV2.SpendMetaCurrency,
			studyAction: FUActorSheetV2.StudyAction,
			action: FUActorSheetV2.PerformAction,
			toggleStatusEffect: FUActorSheetV2.ToggleStatusEffect,
			useEquipment: FUActorSheetV2.ToggleUseEquipment,
			itemFavored: FUActorSheetV2.ToggleItemFavored,
			zenitTransfer: FUActorSheetV2.ZenitTransfer,
			...
			// Buttons
			modifyClassLevel: FUActorSheetV2.modifyClassLevel,
			modifyResource: { handler: FUActorSheetV2.modifyResource, buttons: [0, 2] },
			equipItem: { handler: FUActorSheetV2.equipItem, buttons: [0, 2] },

			// Features
			toggleActiveArcanum: FUActorSheetV2.updateArcanistArcanum,
			toggleActiveGarden: FUActorSheetV2.toggleActiveGarden,
			togglePlantedMagiseed: FUActorSheetV2.togglePlantedMagiseed,
			updatePilotVehicle: FUActorSheetV2.updatePilotVehicle,

			// Active effects
			createEffect: FUActorSheetV2.CreateEffect,
			editEffect: FUActorSheetV2.EditEffect,
			deleteEffect: FUActorSheetV2.DeleteEffect,
			toggleEffect: FUActorSheetV2.ToggleEffect,
			copyInline: FUActorSheetV2.CopyInline,
			rollEffect: FUActorSheetV2.RollEffect,
			clearTempEffects: FUActorSheetV2.ClearTempEffects,
		},
	};
	...
	async _prepareContext(options) {
		...
		// TODO: V2 Sheet Feature
		// Check if guard status effect is active
		const guardEffect = this.actor.effects.find((e) => isActiveEffectForStatusEffectId(e, 'guard'));
		context.guardActive = !!guardEffect;
	...
	_attachFrameListeners() {
		...
		html.addEventListener('keydown', (ev) => {
			if (ev.key === 'Enter' && ev.target.matches('input[type="text"], input[type="number"], textarea')) {
				ev.preventDefault();
				ev.stopPropagation();
				ev.target.blur();
			}
