...
async function prepareItems(context) {
	...
	for (const originalItem of context.items) {
		...
		if (item.type === 'basic') {
			...
			if (originalItem.parent === context.actor) {
				other.push(originalItem);
			}
		}
	}
...
function activateDefaultListeners(html, sheet) {
	...
	if (sheet.actor.isCharacterType) {
		contextMenuOptions.push({
			...
			condition: (jq) => {
				let item = sheet.actor.items.get(jq.data('itemId'));
				if (!item) {
					item = fromUuidSync(jq.data('uuid'));
				}
...
function activateInventoryListeners(html, sheet) {
	...
	html.on('click', '.item-loot', (ev) => onTradeItem($(ev.currentTarget), sheet, false));
