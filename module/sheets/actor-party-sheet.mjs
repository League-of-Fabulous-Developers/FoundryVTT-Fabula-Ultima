...
export class FUPartySheet extends ActorSheet {
	...
	async promptAwardResources() {
		...
		const resources = [
			{ id: 'fp', label: 'FU.FabulaPoints' },
			{ id: 'ip', label: 'FU.InventoryPoints' },
			{ id: 'zenit', label: 'FU.Zenit' },
			{ id: 'hp', label: 'FU.HealthPoints' },
			{ id: 'mp', label: 'FU.MindPoints' },
		];
		...
		const templateData = {
			...
			defaultSource,
			resources,
			localize: (key) => StringUtils.localize(key), // Функция для локализации в шаблоне
		};

		const content = await renderTemplate('systems/projectfu/templates/app/award-resources.hbs', templateData);
		...
		new Dialog({
			...
			buttons: {
				confirm: {
					label: StringUtils.localize('FU.Confirm'),
					callback: async (html) => {
				...
				cancel: {
					label: StringUtils.localize('FU.Cancel'),
				},
			},
			default: 'confirm',
		}).render(true);
